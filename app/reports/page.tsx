              ...
              <div className="space-y-2">
                {flatItems.map((row) => {
                  ...
                  return (
                    <div
                      key={rowKey}
                      className="grid grid-cols-[120px,1fr,96px,96px,120px,120px,120px,72px] items-center gap-2 rounded border bg-white px-4 py-3"
                    >
                      <span className="text-sm font-medium text-gray-900">{row.__dateLabel}</span>
                      <span className="truncate text-sm text-gray-700">{filters.user || '—'}</span>
                      <span className="text-sm text-gray-700 tabular-nums">{row.startJst ?? '—'}</span>
                      <span className="text-sm text-gray-700 tabular-nums">
                        <span>{row.endJst ?? '—'}</span>
                        {row.autoGenerated ? (
                          <span
                            className="badge-auto ml-2 inline-flex h-4 w-4 items-center justify-center rounded-full border-2 border-red-500 bg-red-500 shadow-[0_0_0_2px_rgba(255,255,255,0.95)]"
                      ...
                      <span className="text-right text-sm text-gray-600 tabular-nums">1件</span>
                    </div>
                  );
                })}
              </div>
